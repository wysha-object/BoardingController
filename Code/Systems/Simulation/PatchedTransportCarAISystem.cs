...
namespace BoardingController.Systems.Simulation
{
    public partial class PatchedTransportCarAISystem : GameSystemBase
    {
        ...
        private struct TransportCarTickJob : IJobChunk
        {
            ...
            {
                bool flag = m_PublicTransportVehicleData.TryGetComponent(prefabRef.m_Prefab, out var componentData);
                bool flag2 = m_CargoTransportVehicleData.TryGetComponent(prefabRef.m_Prefab, out var componentData2);
                ...
                if (flag)
                {
                    ...
                    if (
                        odometer.m_Distance >= componentData.m_MaintenanceRange
                ...
                if (
                    flag2
                    && odometer.m_Distance >= componentData2.m_MaintenanceRange
                ...
                if (flag3)
                {
                    ...
                    if (
                        serviceDispatches.Length <= math.select(0, 1, flag4)
                ...
                else if (VehicleUtils.PathEndReached(currentLane) || VehicleUtils.ParkingSpaceReached(currentLane, pathOwner))
                {
                    ...
                    )
                    {
                        if ((cargoTransport.m_State & CargoTransportFlags.Boarding) != 0 || (publicTransport.m_State & PublicTransportFlags.Boarding) != 0)
                        {
                            ...
                            (
                                ...
                                || passengers.Length <= 0
                                || !StartBoarding(jobIndex, vehicleEntity, currentRoute, prefabRef, ref cargoTransport, ref publicTransport, ref target, flag2)
                            )
                            && !SelectNextDispatch(
                                jobIndex,
                                vehicleEntity,
                    ...
                    else if ((cargoTransport.m_State & CargoTransportFlags.Boarding) != 0 || (publicTransport.m_State & PublicTransportFlags.Boarding) != 0)
                    {
                        ...
                        (publicTransport.m_State & (PublicTransportFlags.Evacuating | PublicTransportFlags.PrisonerTransport)) == 0
                        && (!m_RouteWaypoints.HasBuffer(currentRoute.m_Route) || !m_WaypointData.HasComponent(target.m_Target))
                    )
                    {
                        ReturnToDepot(jobIndex, vehicleEntity, currentRoute, owner, serviceDispatches, ref cargoTransport, ref publicTransport, ref car, ref pathOwner, ref target);
                    }
            ...
            {
                if (
                    !m_ConnectedLookup.TryGetComponent(target.m_Target, out var componentData)
                ...
                for (int num = navigationLanes.Length - 1; num >= 0; num--)
                {
                    ...
                    if (carNavigationLane.m_CurvePosition.y - carNavigationLane.m_CurvePosition.x != 0f)
                    {
                        ...
                        float5 = carNavigationLane.m_CurvePosition;
                        break;
                    }
                }
                if (float5.x == float5.y)
                {
                    ...
                    float5 = currentLane.m_CurvePosition.xz;
                }
                if (float5.x != float5.y && m_CurveData.TryGetComponent(entity, out var componentData3))
                {
                    float2 xz = MathUtils.Position(componentData3.m_Bezier, float5.y).xz;
                    float2 xz2 = MathUtils.Tangent(componentData3.m_Bezier, float5.y).xz;
                    if (math.dot(y: componentData2.m_Position.xz - xz, x: MathUtils.Left(math.select(xz2, -xz2, float5.y < float5.x))) > 0f)
                    {
            ...
            )
            {
                ...
                VehicleUtils.ValidateParkingSpace(
                    entity,
                    ref random,
                    ref currentLane,
                    ref pathOwner,
                    ...
                    ref m_GarageLaneData,
                    ref m_ConnectionLaneData,
                    ref m_PrefabRefData,
                    ref m_PrefabParkingLaneData,
            ...
            )
            {
                ...
                {
                    if (
                        m_PrisonData.TryGetComponent(owner.m_Owner, out var componentData)
                ...
                {
                    if (
                        m_EmergencyShelterData.TryGetComponent(owner.m_Owner, out var componentData2)
                        && (componentData2.m_Flags & (EmergencyShelterFlags.HasAvailableVehicles | EmergencyShelterFlags.HasShelterSpace))
            ...
            )
            {
                ...
                else
                {
                    if (previousTarget != target.m_Target)
                    {
                        ...
                        if (
                            m_SlaveLaneData.TryGetComponent(entity, out var componentData)
            ...
            )
            {
                ...
                if (
                    ...
                    && (!m_ConnectedLookup.HasComponent(target.m_Target) || !m_BoardingVehicleData.HasComponent(m_ConnectedLookup[target.m_Target].m_Connected))
                )
                {
                ...
                {
                    Connected connected = m_ConnectedLookup[target.m_Target];
                    if (
                        GetTransportStationFromStop(connected.m_Connected) == Entity.Null
                        && (cargoTransport.m_State & (CargoTransportFlags.RequiresMaintenance | CargoTransportFlags.AbandonRoute)) == 0
                        && (publicTransport.m_State & (PublicTransportFlags.RequiresMaintenance | PublicTransportFlags.AbandonRoute)) == 0
                    )
                    {
                        if (!(m_BoardingVehicleData[connected.m_Connected].m_Testing == vehicleEntity))
                        {
                            if (navigationLanes.Length != 0 && (value.m_Flags & Game.Vehicles.CarLaneFlags.Reserved) == 0)
                            {
                                ...
                                if (
                                    (carNavigationLane.m_Flags & Game.Vehicles.CarLaneFlags.Reserved) == 0
                                    ...
                                    || componentData.m_Owner != componentData2.m_Owner
                                )
                                {
                                    return;
                                }
            ...
            )
            {
                ...
                if ((pathOwner.m_State & PathFlags.Append) == 0)
                {
                    ...
                    VehicleUtils.ResetParkingLaneStatus(
                        vehicleEntity,
                        ref currentLane,
                        ref pathOwner,
                        path,
                        ...
                        ref m_CarLaneData,
                        ref m_ConnectionLaneData,
                        ref m_SpawnLocationData,
                        ref m_PrefabRefData,
                        ref m_PrefabSpawnLocationData
                    );
                }
                VehicleUtils.SetParkingCurvePos(
                    vehicleEntity,
                    ref random,
                    currentLane,
                    pathOwner,
            ...
            private void SetNextWaypointTarget(CurrentRoute currentRoute, ref PathOwner pathOwnerData, ref Target targetData)
            {
                ...
                VehicleUtils.SetTarget(
                    ...
                    ref targetData,
                    dynamicBuffer[
                        WaypointUtils.SelectNextWaypoint(m_ConnectedLookup, m_CustomWaypointLookup, dynamicBuffer, m_WaypointData[targetData.m_Target].m_Index)
                    ].m_Waypoint
                );
            }
            ...
            )
            {
                ...
                if (m_ConnectedLookup.HasComponent(target.m_Target))
                {
                    Connected connected = m_ConnectedLookup[target.m_Target];
            ...
            )
            {
                ...
                if (!forcedStop)
                {
                    ...
                    publicTransport.m_MinWaitingDistance = float.MaxValue;
                    if (
                        (flag || (publicTransport.m_State & (PublicTransportFlags.Evacuating | PublicTransportFlags.PrisonerTransport)) != 0)
                        && (
            ...
            private Entity GetNextStorageCompany(Entity route, Entity currentWaypoint)
            {
                ...
                for (int i = 0; i < dynamicBuffer.Length; i++)
                {
                    ...
                    if (m_ConnectedLookup.HasComponent(waypoint))
                    {
                        Entity connected = m_ConnectedLookup[waypoint].m_Connected;
        ...
        protected override void OnCreate()
        {
            ...
            m_EvacuationRequestArchetype = base.EntityManager.CreateArchetype(
                ComponentType.ReadWrite<ServiceRequest>(),
            ...
            m_MovingToParkedCarRemoveTypes = new ComponentTypeSet(
        ...
        protected override void OnUpdate()
        {
            ...
            JobHandle jobHandle = JobChunkExtensions.ScheduleParallel(
                new TransportCarTickJob
                {
                    ...
                    m_PathInformationType = SystemAPI.GetComponentTypeHandle<PathInformation>(true),
                    ...
                    m_CarNavigationLaneType = SystemAPI.GetBufferTypeHandle<Game.Vehicles.CarNavigationLane>(false),
                    m_ServiceDispatchType = SystemAPI.GetBufferTypeHandle<Game.Simulation.ServiceDispatch>(false),
                    m_EntityLookup = SystemAPI.GetEntityStorageInfoLookup(),
                    m_ParkedCarData = SystemAPI.GetComponentLookup<Game.Vehicles.ParkedCar>(true),
                    ...
                    m_PathInformationData = SystemAPI.GetComponentLookup<Game.Pathfind.PathInformation>(true),
                    ...
                    m_PublicTransportVehicleData = SystemAPI.GetComponentLookup<Game.Prefabs.PublicTransportVehicleData>(true),
                    m_CargoTransportVehicleData = SystemAPI.GetComponentLookup<Game.Prefabs.CargoTransportVehicleData>(true),
                    ...
                    m_ServiceRequestData = SystemAPI.GetComponentLookup<Game.Simulation.ServiceRequest>(true),
                    m_TransportVehicleRequestData = SystemAPI.GetComponentLookup<Game.Simulation.TransportVehicleRequest>(true),
                    m_EvacuationRequestData = SystemAPI.GetComponentLookup<Game.Simulation.EvacuationRequest>(true),
                    m_PrisonerTransportRequestData = SystemAPI.GetComponentLookup<Game.Simulation.PrisonerTransportRequest>(true),
                    m_WaypointData = SystemAPI.GetComponentLookup<Game.Routes.Waypoint>(true),
                    m_BoardingVehicleData = SystemAPI.GetComponentLookup<Game.Routes.BoardingVehicle>(true),
                    m_RouteLaneData = SystemAPI.GetComponentLookup<Game.Routes.RouteLane>(true),
                    ...
                    m_PrisonData = SystemAPI.GetComponentLookup<Game.Buildings.Prison>(true),
                    m_EmergencyShelterData = SystemAPI.GetComponentLookup<Game.Buildings.EmergencyShelter>(true),
                    ...
                    m_GarageLaneData = SystemAPI.GetComponentLookup<Game.Net.GarageLane>(true),
                    ...
                    m_PathElements = SystemAPI.GetBufferLookup<Game.Pathfind.PathElement>(false),
                    m_LoadingResources = SystemAPI.GetBufferLookup<Game.Vehicles.LoadingResources>(false),
                    ...
                    m_ConnectedLookup = SystemAPI.GetComponentLookup<Game.Routes.Connected>(true),
